...
3. **Interactive tree and quick view** ✅: the Tree tab, a pan-and-zoom canvas of everyone you can see, centred on you; automatic generation layout; couple / parent / step / adopted / foster line styles with a key and a relationships toggle; search, branch filter, tap for a quick view, add relatives from the tree; "Show on tree" from a profile. Adding a partner asks whether they're also the parent of the person's children, and a step-parent can be made a parent when their partner is the only parent recorded
4. **How are we related?**: relationship naming (up to "second cousin twice removed", step relatives, partners, in-laws), shortest path shown on the tree, "relation to you" labels throughout
5. **Home dashboard and set-up flow**: generations, *On this day*, recently added with relation labels, "Plant your family tree" onboarding
6. **Photos and genetic conditions**: uploads (cover, profile, gallery); conditions with blood-relative visibility and inherited "watch" items
